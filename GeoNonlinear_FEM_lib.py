...
def B2D_SR_ML(ue, EA, EI, GAq, l):
    ...
    # auxiliary variables
    Gamma = (2*phi1**2 - phi1*phi2 + 2*phi2**2)*l**2 + ((3*w1 - 3*w2)*phi1 + (3*w1 - 3*w2)*phi2 + 30*u1 - 30*u2)*l + 18*(w1 - w2)**2
    Omega = 12/l**3*w1 + 6/l**2*phi1 - 12/l**3*w2 + 6/l**2*phi2
    Zeta = -6/l**2*w1 - 2/l*phi1 + 6/l**2*w2 - 4/l*phi2
    ...
    fine = np.array([
         EA * Gamma / (30*l**2),
         EI * Omega + EA * Phi * Gamma / (900*l**3),
         EI * l * Omega + EI * Zeta + EA * Psi * Gamma / (900*l**3),
        -EA * Gamma / (30*l**2),
        -EI * Omega - EA * Phi * Gamma / (900*l**3),
        -EI * Zeta + EA * Xi * Gamma / (900*l**3)
    ])
    ...
    # mean axial strain
    esp0_m = ((2 * phi1 ** 2 - phi1 * phi2 + 2 * phi2 ** 2) * l ** 2 + ((3 * w1 - 3 * w2) * phi1 + (3 * w1 - 3 * w2) * phi2 + 30 * u1 - 30 * u2) * l + 18 * (w1 - w2) ** 2) / l ** 2 / 30
    # axial force
    N = EA * esp0_m
    # geometric stiffness matrix
    kge = np.array([
        [0,                0,                0,   0,                0,                 0],
        [0,  (6 / 5) * N / l,           N / 10,   0, -(6 / 5) * N / l,            N / 10],
        [0,           N / 10, (2 / 15) * N * l,   0,          -N / 10,       -N * l / 30],
        [0,                0,                0,   0,                0,                 0],
        [0, -(6 / 5) * N / l,          -N / 10,   0,  (6 / 5) * N / l,           -N / 10],
        [0,           N / 10,      -N * l / 30,   0,          -N / 10,  (2 / 15) * N * l]
    ])
